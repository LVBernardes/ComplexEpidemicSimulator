...
from complex_epidemics.agents.support_objects.human.human_occupation_categories import (
    ...
    StudentCategory,
    TransportCategory, WorkerCategory,
)
...
class Occupation(IModelStepper):
    ...
    def __init__(self) -> None:
        ...
        self._locale: Any = None
        self._is_locale_assigned: bool = False
    ...
    @property
    def is_locale_assigned(self) -> bool:
        return self._is_locale_assigned

    @is_locale_assigned.setter
    def is_locale_assigned(self, value: bool) -> None:
        self._is_locale_assigned = value
    ...
    def assign_occupation_locale(self) -> None:
        ...
        for locale_class in locales_classes:
            ...
            if real_locale_id_list:
                while not self.is_locale_assigned and loop_counter < 10:
                    real_locale_id = rng.choice(real_locale_id_list)
                    real_locale = self._human.model.get_agent_by_id(real_locale_id)
                    loop_counter += 1
                    if (
                        len(real_locale.allowed_occupations)
                        ...
                        real_locale.add_occupation_individual(self._human.graph_node_id)
                        self.locale = real_locale
                        self.is_locale_assigned = True
                        return None
                if not self.is_locale_assigned:
...
class TransportDriver(Occupation):
    def __init__(self) -> None:
        super().__init__()
        self._category: TransportCategory = TransportCategory.GENERIC
        self._is_economic_active: bool = True
        self._activities: list = []
